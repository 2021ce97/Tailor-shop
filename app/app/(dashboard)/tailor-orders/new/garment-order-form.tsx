...
export function GarmentOrderForm({ locale, title, customers, garmentTypes, fabrics }: { locale: "en" | "fa" | "ps"; title: string; customers: SearchableOption[]; garmentTypes: GarmentType[]; fabrics: { id: number; name: string; color: string | null; unit: string; stockQty: string; sellingPricePerUnit: string }[] }) {
  ...
  const text = locale === "en"
    ? { customer: "Customer", newCustomer: "New customer", phone: "Contact number", add: "Add garment", items: "Garments", measure: "Measurements", design: "Design", price: "Price", date: "Date", delivery: "Delivery date", advance: "Advance paid", create: "Create order", notes: "Notes", fabric: "Fabric (shop inventory)", fabricQty: "Fabric quantity used", help: "Record measurements and design choices, then add each garment as a separate ticket.", search: "Search name, phone, or code", garmentType: "Garment type", addOne: "Add at least one garment.", customerFabric: "Customer provides fabric" }
    : locale === "fa"
      ? { customer: "مشتری", newCustomer: "مشتری جدید", phone: "شماره تماس", add: "افزودن لباس", items: "لباس‌ها", measure: "اندازه‌ها", design: "دیزاین", price: "قیمت", date: "تاریخ", delivery: "تاریخ تحویل", advance: "پیش‌پرداخت", create: "ثبت سفارش", notes: "یادداشت", fabric: "پارچه (موجودی دوکان)", fabricQty: "مقدار پارچه مصرفی", help: "اندازه‌ها و انتخاب‌های دیزاین را ثبت کنید؛ سپس هر لباس را به‌عنوان یک تکت جداگانه اضافه کنید.", search: "نام، تماس یا کد را جستجو کنید", garmentType: "نوع لباس", addOne: "حداقل یک لباس اضافه کنید.", customerFabric: "پارچه را مشتری می‌آورد" }
      : { customer: "پېرېدونکی", newCustomer: "نوی پېرېدونکی", phone: "د اړیکې شمېره", add: "لباس زیاتول", items: "لباسونه", measure: "اندازې", design: "ډیزاین", price: "بیه", date: "نېټه", delivery: "د سپارلو نېټه", advance: "پیشکي", create: "سپارښتنه ثبتول", notes: "یادښت", fabric: "ټوکر (د دوکان زېرمه)", fabricQty: "د کارېدونکي ټوکر مقدار", help: "اندازې او د ډیزاین انتخابونه ثبت کړئ، بیا هر لباس د جلا ټکټ په توګه زیات کړئ.", search: "نوم، ټیلیفون یا کوډ ولټوئ", garmentType: "د جامو ډول", addOne: "لږ تر لږه یو لباس زیات کړئ.", customerFabric: "ټوکر پېرېدونکی راوړي" };
  function addItem() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    <div className="rounded-3xl border border-slate-200 bg-gradient-to-r from-violet-50 via-slate-50 to-fuchsia-50 p-6 shadow-sm"><h1 className="text-xl font-semibold text-slate-900">{title}</h1><p className="mt-1 text-sm text-slate-600">{text.help}</p></div>
    <form action={action} className="space-y-5">
      ...
      <section className="grid gap-4 rounded-xl border border-slate-200 bg-white p-5 md:grid-cols-3">
        <SearchableSelect name="customerSearch" label={text.customer} options={customers} onSelect={(option) => setCustomerId(option?.value ?? null)} placeholder={text.search} />
        ...
        <label className="flex flex-col gap-1 text-sm"><span>{text.fabric}</span><select name="fabricId" defaultValue="" className="rounded-md border border-slate-300 px-3 py-2"><option value="">{text.customerFabric}</option>{fabrics.filter((fabric) => Number(fabric.stockQty) > 0).map((fabric) => <option key={fabric.id} value={fabric.id}>{fabric.name}{fabric.color ? ` · ${fabric.color}` : ""} ({fabric.stockQty} {fabric.unit})</option>)}</select></label>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5"><div className="mb-3 flex justify-between"><h2 className="font-semibold">{text.items}</h2><strong>{total.toFixed(2)} AFN</strong></div>{items.length === 0 ? <p className="text-sm text-slate-500">{text.addOne}</p> : <div className="space-y-2">{items.map((item, index) => <div key={index} className="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 text-sm"><span>{garmentTypes.find((type) => type.id === item.garmentTypeId)?.name}</span><span>{item.amount.toFixed(2)} AFN <button type="button" className="ms-3 text-red-600" onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}>×</button></span></div>)}</div>}</section>
